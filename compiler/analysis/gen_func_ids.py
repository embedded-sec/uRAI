...
if __name__ == "__main__":
    ...
    if args.test:
        '''
        #ifc = [0, 1, 2, 4, 6, 7, 9, 10, 11]
        ifc = range(0, 4096, 4)
        valid_vals = range(2, 4095, 4)
        check_valid = True
        for k in range(2, 4095, 4):
            iff = []
            for f in ifc:
                k_f = k ^ f
                if k_f not in valid_vals:
                    check_valid = False
                iff.append(k_f)
            #print("-"*20)
            #print("k = %d, out_flags = %s" % (k, iff))
            #print("-"*20)
        k_size = 0
        val = 0
        for i in range(2, 15, 4):
            k_size += 1
            val = i
        print("Validity check: ", check_valid)
        print("size: ", k_size)
        print(val)
        return_val = subprocess.call("ls -l", shell=True)
        print(return_val)
        ...
        func1 = FuncNode("funny1")
        func2 = FuncNode("funny2")
        func3 = FuncNode("funny3")
        func4 = FuncNode("funny4")
        func5 = FuncNode("funny5")
        func1.sr_shift = 7
        func2.sr_shift = 5
        func3.sr_shift = 5
        func4.sr_shift = 10
        ...
        func1.shifted_flt_size = 100
        func2.shifted_flt_size = 50
        func3.shifted_flt_size = 20
        func4.shifted_flt_size = 1
        func5.shifted_flt_size = 10
        func_list = [func1, func3, func5, func4, func2]
        print("List: ")
        for f in func_list:
            print(f.name)
        func_list = sort_func_list(func_list)
        ...
        ----------------------
        stdlib_json = {}
        with open("std_clib_func_list.txt", 'r') as fd:
            ...
            for line in file_lines:
                ...
                if line[0]:
                    ...
                    print("func: %s" % func)
                    print("cntr:", cntr)
                    if "wint_t" in func or "wchar_t" in func or func == "int" \
                            or "*" in func:
                        func = line[1].split("(")[0].split("*")
                        func = func[len(func)-1]
                    if cntr != 0 and ".h" not in func and func:
                        stdlib_json[str(func)] = "STDLIB_FUNCTION"
                        print("PASSED")
                    print("---------------------------------------")
                cntr += 1
        ...
        # write the results to a file
        with open('STD_LIB.json', 'w') as stdlib_fd:
            json.dump(stdlib_json, stdlib_fd, sort_keys=True, indent=4,
                      ensure_ascii=False)
        test_name1 = "memset"
        test_name2 = "Func8"
        if is_unsupported_lib_func(test_name1):
            print("%s is a cstdlib function" % test_name1)
        if is_unsupported_lib_func(test_name2):
            print("%s is a cstdlib function" % test_name2)
        '''
        #print(STDLIB_FILE)
        eff = "eff"
        shift = "shift"
        ...
        l2 = 64
        l3 = 256
        test_dict = {l1: {eff: 0.9, shift: 5}, l2: {eff: 0.95, shift: 10},
                     l3: {eff: 0.6, shift: 0}}
