...
import setup_asm_and_ld as helpers
...
LLVM_PATH = None
URAI_LIB = None
URAI_KEY_FLAG_IDS_TOOL = None
URAI_FINAL_LD_TOOL = None
URAI_RESULT_TOOL = None
...
def build_LUTS():
    ...
    #    'STM32(F|L)3': '-mthumb -mcpu=cortex-m4 -mfpu=fpv4-sp-d16 -mfloat-abi=hard',
        'STM32(F|L)3': '-mcpu=cortex-m4',
    #    'STM32(F|L)4': '-mthumb -mcpu=cortex-m4 -mfpu=fpv4-sp-d16 -mfloat-abi=hard',
        'STM32(F|L)4': '-mcpu=cortex-m4',
        'STM32(F|L)7': '-mcpu=cortex-m7',# -mfloat-abi=hard',
    }
    ...
    mcu_regex_to_libs_dict = {
        ...
        #    'STM32(F|L)4': '-mthumb -mcpu=cortex-m4 -mfpu=fpv4-sp-d16 -mfloat-abi=hard',
            ...
            'STM32(F|L)7': '-L %s/arm-none-eabi/lib/thumb/v7e-m -L %s/lib/gcc/arm-none-eabi/6.3.1/thumb/v7e-m'%(GCC_PATH,GCC_PATH),# -mfloat-abi=hard',
    }


    mcu_regex_to_comp_support_dict = {
            'STM32(F|L)0': 'urai-rt-v6-m.o',
            'STM32(F|L)1': 'urai-rt-v7-m.o',
            'STM32(F|L)2': 'urai-rt-v7-m.o',
        #    'STM32(F|L)3': thumb -mcpu=cortexsp-d16 -mfloat-abi=hard',
            'STM32(F|L)3': 'urai-rt-v7e-m.o',
        #    'STM32(F|L)4': thumb -mcpu=cortexsp-d16 -mfloat-abi=hard',
            'STM32(F|L)4': 'urai-rt-v7e-m.o',
            'STM32(F|L)7': 'urai-rt-v7e-m.o',# -mfloat-abi=hard',
    }
...
def main():
    ...
    if len(sys.argv) != 4:
        ...
        sys.stderr.write("\nUsage:\n")
        sys.stderr.write("  CubeMX2Makefile.py <SW4STM32 project folder> <URAI ROOT> <Project Name>\n")
    ...
    # Configuration
    urai_root_path = os.path.abspath(sys.argv[2])
    print urai_root_path
    if not (os.path.isdir(urai_root_path)):
        sys.stderr.write("URAI \"Root of URAI Tools not found not found: \n")
        sys.exit(C2M_ERR_INVALID_COMMANDLINE)
    GCC_PATH = os.path.join(urai_root_path,'gcc','bins/')
    LLVM_PATH = os.path.join(urai_root_path,'llvm','build')
    URAI_LIB = os.path.join(urai_root_path,'llvm','urai-rt-lib')
    URAI_KEY_FLAG_IDS_TOOL = os.path.join(urai_root_path,'compiler','analysis','gen_func_ids.py')
    #URAI_FINAL_LD_TOOL  = os.path.join(urai_root_path,'compiler','graph_analysis','final_linker_gen.py')
    #URAI_RESULT_TOOL = os.path.join(urai_root_path,'compiler','graph_analysis','collect_results.py')
    build_LUTS()
    ...
    # C_INCLUDES
    include_nodes =root.findall('.//tool[@superClass="fr.ac6.managedbuild.tool.gnu.cross.c.compiler"]/option[@valueType="includePath"]/listOptionValue')
    ...
    for include_node in include_nodes:
        ...
        if len(c_include_str .strip()) > 0:
            inc_dir = c_include_str[c_include_str.find('/')+1:] #remove one ../ because this isn't made in a dir
    ...
    # ASM_INCLUDES
    include_nodes =root.findall('.//tool[superClass="fr.ac6.managedbuild.tool.gnu.cross.assembler"]/option[@valueType="includePath"]/listOptionValue')
    #print "Include Nodes", include_nodes
    for include_node in include_nodes:
    ...
    for mcu_regex_pattern, cflags in mcu_regex_to_cflags_dict.items():
        if re.match(mcu_regex_pattern, mcu_str):
            ...
            ld_subst = cflags
            break
    for mcu_regex_pattern, float_abi in mcu_regex_to_float_abi_dict.items():
        if re.match(mcu_regex_pattern, mcu_str):
            float_abi_subs = float_abi
            break
    for mcu_regex_pattern, libs in mcu_regex_to_libs_dict.items():
        if re.match(mcu_regex_pattern, mcu_str):
            std_lib_subs = libs
            break

    for mcu_regex_pattern, lib in mcu_regex_to_comp_support_dict.items():
        if re.match(mcu_regex_pattern, mcu_str):
            rt_lib = URAI_LIB + "/" + lib
            break

    for mcu_regex_pattern, lib in mcu_regex_to_syscalls_dict.items():
        if re.match(mcu_regex_pattern, mcu_str):
            syscalls_lib = URAI_LIB +"/"+ lib
            break
    ...
    helpers.add_magic_strs_to_linker(ld_script_name)
    ld_script_subst = 'LDSCRIPT = {}'.format(ld_script_name)


    # Get libraries
    lib_dir_nodes = root.findall('.//tool/option[@superClass="gnu.c.link.option.paths"]/listOptionValue')
    library_dirs = []
    #print lib_dir_nodes
    ...
    #        dirs = int(s) for s in str.split() if s.isdigit()
            ...
            if filename:
                ...
                if filename.endswith('.c'):
                    ...
                    if filename.endswith("stm32f4xx_it.c"):
                        helpers.fix_up_handlers(filename)
                if filename.endswith('.s'):
                    helpers.fix_up_asm(filename)
    ...
    makefile_str = makefile_template.substitute(
        ...
        LDMCU = ld_subst,
        URAI_RT_LIB = rt_lib,
        CLANG_SYSCALLS_LIB = syscalls_lib,
        ...
        URAI_KEY_FLAG_IDS_TOOL = URAI_KEY_FLAG_IDS_TOOL,
        URAI_FINAL_LD_TOOL = URAI_FINAL_LD_TOOL,
        URAI_RESULT_TOOL = URAI_RESULT_TOOL,
