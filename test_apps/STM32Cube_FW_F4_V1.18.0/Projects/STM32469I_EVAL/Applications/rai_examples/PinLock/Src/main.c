...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
__attribute__((used,optnone)) void __start_measurement(void){
  __asm("nop\n");
}

__attribute__((used,optnone)) void __end_measurement(void){
  __asm("bkpt\n");
}
...
void rx_from_uart(uint32_t size){
    ...
    //for (uint8_t i = 0; i < size; i++){
      ...
      //UartReady = HAL_UART_STATE_RESET;//RESET;
    //  PinRxBuffer[i] = (char)recv_buff[0];
    //}
